...
const SYSTEM_PROMPT = `You are an EHS (Environmental Health and Safety) workflow design expert.

When given a process description, generate a structured EHS workflow as a JSON array of steps.
  ...
  "steps": [
    {
      ...
      "owner": "Role responsible (e.g. EHS Manager, Site Lead, HR, Facilities, Safety Officer, First Aider)",
...
- SLA hours should reflect real-world EHS urgency (immediate response steps = 1–4h, documentation = 24–48h, training = 72–168h).
- Owners should be realistic EHS team roles, not individual names.
- Steps should reflect actual EHS/OSHA best practices where applicable.
...
export async function POST(req: NextRequest) {
  ...
  const message = await client.chat.completions.create({
    ...
    messages: [
      ...
      { role: "user", content: `Generate an EHS workflow for the following process:\n\n${description}` },
    ],
  });
